...
import { CircleDotIcon, FileTextIcon, Link2Icon, TypeIcon } from 'lucide-react';
...
type ServiceFormProps = {
    form: InertiaFormProps<ServiceFormValues>;
...
export function ServiceForm({ form, onSubmit, onCancel, onImageClear, mode = 'create', imageUrl }: ServiceFormProps) {
    ...
    const [touched, setTouched] = useState<Record<keyof ServiceFormValues, boolean>>({
        title_en: false,
        title_zh: false,
        title_my: false,
    ...
    const markTouched = (field: keyof ServiceFormValues) => {
    ...
    const fieldState = (field: keyof ServiceFormValues): 'idle' | 'error' | 'success' => {
        if (!touched[field] && !submitted) {
            return 'idle';
        }

        return form.errors[field] || validateServiceField(field, form.data, t) ? 'error' : 'success';
    };

    const fieldError = (field: keyof ServiceFormValues): string | undefined => {
        if (!touched[field] && !submitted) {
            return undefined;
        }

        return form.errors[field] || validateServiceField(field, form.data, t);
    };
    ...
    const submit = (event: FormEvent) => {
        ...
        const errors = validateService(form.data, t);
    ...
    return (
        <CmsFormShell onSubmit={submit} onCancel={onCancel} processing={form.processing} mode={mode}>
            <div>
                ...
                <FormField
                    label={t('cms.service.description_en')}
            ...
            <div>
                ...
                <FormField
                    label={t('cms.service.description_zh')}
            ...
            <div className="md:ml-3">
                ...
                <FormField
                    label={t('cms.service.description_my')}
